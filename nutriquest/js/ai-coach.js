...
async function handleUserMessage(msg) {
    ...
    try {
        // SAFETY: Handle null profile (e.g. if specific field missing or load failed)
        const profile = userProfile || { age: 'Unknown', weight: 'Unknown', goal: 'General Health' };

        // Construct Context-Aware Prompt
        const systemContext = `You are a professional Nutritionist and Fitness Coach. 
        User Profile: Age ${profile.age || 'Unknown'}, Weight ${profile.weight || '?'}kg, Goal: ${profile.goal || 'General Health'}.
        ...
        // BYPASS: For presentation, if ANY error occurs, switch to Demo AI Mode
        // This covers 404 (Model not found), 429 (Quota), 500 (Server), etc.
        const demoResponse = "✨ **Demo AI Mode**\n\n*Connection Note: " + error.message + "*\n\n" +
            "To support your goal of **" + (userProfile?.goal || 'General Health') + "**, I recommend:\n\n" +
            "1. **Prioritize Protein**: Aim for 20-30g per meal.\n" +
            "2. **Hydration**: Drink 3L of water daily.\n" +
            "3. **Sleep**: 7-8 hours for recovery.\n\n" +
            "*I am ready to generate your full plan now!* 💪";

        addMessageBubble(demoResponse, 'ai');
...
async function generateFullPlan() {
    ...
    try {
        ...
        // Safety Profile
        const profile = userProfile || { age: '25', weight: '70', goal: 'General Fitness' };

        const systemPrompt = `Generate a dedicated 7-Day Workout & Diet Plan. 
        User: Age ${profile.age || '25'}, Weight ${profile.weight || '70'}kg, Goal: ${profile.goal || 'General Fitness'}.
        ${preferencesContext}
