...
async function loadDashboardData() {
    try {
        ...
        if (!user) {
            console.warn('⚠️ User data not available after retry');
            // Try explicit fetch
            if (auth.currentUser) {
        ...
        if (!user || !user.goals) {
            console.error('❌ User goals missing, cannot load dashboard');
            return;
        }
        ...
        // 2. Load Today's Meals with Fallback
        let meals = await getTodaysMeals();

        // If empty, try fetching recent to see if ANY data exists (helps debug connection)
        if (!meals || meals.length === 0) {
            console.log('ℹ️ No meals for today found. Checking recent history...');
            const recent = await getRecentMeals(1);
            if (recent.length > 0) {
                // Check if the most recent meal is actually today (timezone edge cases)
                const today = new Date().toDateString();
                const recentDate = new Date(recent[0].timestamp).toDateString();
                if (today === recentDate) {
                    console.log('⚠️ Found meal in recent history that belongs to today. Refining fetch.');
                    // Just use it? Or maybe getTodaysMeals logic is strict on time.
                    // Let's re-fetch recent 20 and filter manually to be safe
                    const recent20 = await getRecentMeals(20);
                    meals = recent20.filter(m => new Date(m.timestamp).toDateString() === today);
                }
            }
        }

        console.log(`✅ Loaded ${meals.length} meals for today`);
